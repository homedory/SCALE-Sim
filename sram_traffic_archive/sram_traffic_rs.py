        ...
        for oh_fold in range(num_oh_fold):
            ...
            for ow_fold in range(num_ow_fold):
                ...
                for curr_filt in range(num_filt):
                    ...
                    cycle = gen_write_trace(lmcol_cycle, curr_filt,
                                            ofmap_h, ofmap_w,
                                            orow_list, ocol_list,
                    ...
                    if energy_counter:
                        ...
                        energy_counter.increment_rf_read((use_arr_h * use_arr_w) * filt_w * ocol_needs) 
                        # RF read for ifmap (for each PE)
                        energy_counter.increment_rf_read((use_arr_h * use_arr_w) * filt_w * ocol_needs)   
                        # RF read for psum (for each PE)
                        energy_counter.increment_rf_read((use_arr_h * use_arr_w) * filt_w * ocol_needs) 
                        # ALU ops
                        energy_counter.increment_alu_operations((use_arr_h * use_arr_w) * filt_w * ocol_needs)
                        # RF write for psum (for each PE)
                        energy_counter.increment_rf_write((use_arr_h * use_arr_w) * filt_w * ocol_needs)


                        # Energy for psum accumulation
                        # PE data transfer 
                        energy_counter.increment_pe_data_transfer((use_arr_h - 1) * use_arr_w)
                        # RF read for psum stored in PE (for psum accumulation)
                        energy_counter.increment_rf_read((use_arr_h - 1) * use_arr_w) 
                        # ALU ops
                        energy_counter.increment_alu_operations((use_arr_h - 1) * use_arr_w)


                        # SRAM write for psum (result of accumulation of psums)
                        energy_counter.increment_sram_write(use_arr_w * ocol_needs)
                        # SRAM read for previous psum (for psum accumulation)
                        if chn > 0:
                            # energy_counter.increment_sram_read(use_arr_w)
                            # ALU ops for psum writeback
                            energy_counter.increment_alu_operations(use_arr_w * ocol_needs) 
                    
                    
                    
                    # Utils
                    delta_cycle = cycle - prev_cycle
                    util_this_fold: float = (use_arr_h * use_arr_w) / (arr_h * arr_w)
...
# Ifmap req address
    ...
    if energy_counter:
        energy_counter.increment_sram_read(sram_reads_cnt)
